...
import math
import itertools
from jrdb_reasoning.Category import all_human_pose_action,OBJECTS,HUMAN_HUMAN_INTERACTION, HUMAN_OBJECT_INTERACTION, GEOMETRY, SOCIAL_AIMS, SALIENT_OBJECTS, BODY_CONTENTS, GENDERS, AGES, RACES, ACTIONS,all_human_pose_action_postfix
from jrdb_reasoning.Category import age_corresponding, object_corresponding,object_specific_corresponding,class_thing_id,all_class_id
from jrdb_reasoning.Category import *
...
def Position_relative_to_Ref(B1,B2):
    ...
    if distance_Z>1.5:
        ...
        if -30 <= theta_signed_deg <= 30:
            ...
            relation = f'{Z_dimention} right left'
        elif 45 < theta_signed_deg <= 135:
            relation = f'{Z_dimention} right'
        elif 135 < theta_signed_deg <= 145:
            relation = f'{Z_dimention} back right'
...
def get_all_combinations(S, T, target_Q):
    ...
    if S == '1':
        ...
        if target_Q == 'human':
            ...
            #         has_hhg = 'hhg' in comb_set
            #         has_hhi = 'hhi' in comb_set
            #         has_action = 'action' in comb_set
            #
            #         # Rule 1: exactly one of hhg or hhi must be present
            #         if not (has_hhg ^ has_hhi):
            #             continue
            #
            #         # Rule 2: hhi cannot appear with action
            #         if has_hhi and has_action:
            #             continue
            #
            #         all_combinations.append(list(comb))
            attributes = ['age', 'race', 'gender', 'action', 'hhg', 'hhi']
